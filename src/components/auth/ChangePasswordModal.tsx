  ...
  return (
    <AnimatePresence>
      ...
      <motion.div
        ...
        className="w-full max-w-md rounded-3xl shadow-2xl border p-6 bg-surface/95 backdrop-blur-xl border-border-subtle text-content-primary overflow-hidden"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-surface-raised border border-border-subtle text-content-primary text-xs focus:outline-none focus:ring-4 focus:ring-cyan-500/10 focus:border-cyan-500/60 transition-all"
              />
            </div>
          </div>
          ...
          <div className="pt-3 border-t border-border-subtle flex items-center justify-end gap-3 mt-5">
            ...
            <motion.button
              whileHover={{ scale: isSubmitting || !isLengthOk || !isMatchOk ? 1 : 1.02 }}
              whileTap={{ scale: isSubmitting || !isLengthOk || !isMatchOk ? 1 : 0.97 }}
              type="submit"
              disabled={isSubmitting || !isLengthOk || !isMatchOk}
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 disabled:opacity-50 text-white font-bold text-xs shadow-lg shadow-cyan-500/20 transition-colors flex items-center gap-2"
            >
              {isSubmitting ? (
                <>
